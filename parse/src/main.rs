...
fn main() -> std::io::Result<()> {
    let file = File::open(r"C:\Users\matth\repo\web\html")?;
    let mut buf_reader = BufReader::new(file);
    let mut text = String::new();
    buf_reader.read_to_string(&mut text)?;


    let title = Regex::new(r#"(?m)<h1 class="sage-page-heading__title">[\s]*\d*.-.Uke.\d*.-.(.*)Report"#).unwrap(); 
    ...
    let list: Vec<Regex> = vec![title,sent, delivered, opened, clicked, hard_bounced, unsubscribed];
    
    for item in list{
        let caps = item.captures_iter(&text);
        for mat in caps {
            println!("{}", mat.get(1).map_or("", |m| m.as_str()));
        }
    }
